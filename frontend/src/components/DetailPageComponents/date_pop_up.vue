<template>
    <div class="calendar-container" v-if="show">
        ...
        <v-calendar borderless expanded :attributes="attributes" @dayclick="onDayClick"/>
        <div class="btn-group">
            <button class="clear" @click="clearDate">Clear date</button>
            <button class="close" @click="$emit('close'); $emit('update-date', selectedDate)">Close</button>
        </div>
    </div>
</template>
