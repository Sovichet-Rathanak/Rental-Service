...
export const useUserStore = defineStore("user", {
  ...
  actions: {
    async createUser(userData) {
      try {
        const userRsp = await axios.post(
    ...
    async handlePfp(event) {
      ...
      try {
        const pfpRsp = await axios.post(
          `http://localhost:3000/api/user/${this.user.id}/profile-picture`,
          formData,
          {
    ...
    async fetchCurrentUser() {
      ...
      try {
        ...
        if (this.authToken) {
          ...
          rsp = await axios.get(
            `http://localhost:3000/api/user/email/${this.user.email}`
          );
        }
    ...
    async updateUser(id, data) {
        try {
            const res = await axios.patch(`http://localhost:3000/api/user/${id}`, data)
            return res.data
        } catch (error) {
