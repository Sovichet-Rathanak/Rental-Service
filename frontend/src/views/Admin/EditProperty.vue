<template>
  <div class="container">
    <div class="top">
      <ImageInForm title="Edit Property" imageUrl="/src/assets/images/property_images/property1.jpeg" text="Add Images"
        :backRoute="{ name: 'Admin Dashboard Property' }" @image-uploaded="handleImage"></ImageInForm>

      <button>Save</button>
    </div>

    <DynamicForm class="form" icon="material-symbols:arrow-back-rounded" :inputSet="RoomInputs"
    ...
    <div class="buttons-container">
      ...
      <div class="fave-amen-container">
        <StepButton v-for="(amenity, index) in guestFavorites" :key="'fave-' + index" :label="amenity.label"
          :icon="amenity.icon" :active="selectedAmenities.includes(amenity)" @setActive="toggleAmenity(amenity)">
        </StepButton>
      </div>
